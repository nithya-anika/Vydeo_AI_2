...
async function renderCloud(params: RenderParams): Promise<RenderResult> {
  ...
  console.log(`[Remotion Lambda] Render accepted. ID: ${renderId} in bucket ${bucketName}`);
  ...
  for (let i = 0; i < maxPolls; i++) {
    ...
    if (progress.done && progress.outputFile) {
      console.log(`[Remotion Lambda] Render complete!`);
      return {
      ...
      throw new Error(`Remotion Lambda render failed: ${progress.errors[0]?.message || "Unknown error"}`);
    } else {
      console.log(`[Remotion Lambda] Polling... Progress: ${Math.round(progress.overallProgress * 100)}%`);
    }
  }

  throw new Error("Remotion Lambda render timed out after 10 minutes.");
}
